...
public class MainActivity extends AppCompatActivity {
    ...
    // ◆画面をタッチしたかしてないかの判定
    private boolean jump_flg = false;

    //◆画面の右側をタッチしているかの判定
    private boolean forward_flg = false;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (start_flg == false) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN) {
                ...
                //◆action_flg = true;
                //◆触れたのが画面の右側なら前進
                if(event.getX() > Math.round(screenWidth / 2)){
                    forward_flg = true;
                }
                //◆触れたのが画面の左側なら後退
                if(event.getX() < Math.round(screenWidth / 2)){
            ...
            if (event.getAction() == MotionEvent.ACTION_POINTER_UP) {
                //◆もう片方の指が画面に触れたら jump_flg を true
                jump_flg = true;

            }
           /* if (event.getAction() == MotionEvent.ACTION_POINTER_DOWN) {
                //◆片方の指を画面から離したら jump_flg を false
                jump_flg = false;
